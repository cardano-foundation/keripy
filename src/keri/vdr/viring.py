...
class Reger(dbing.LMDBer):
    ...
    def removeTransactionEscrows(self, pre, said):
        ...
        anchorless/out-of-order processors from retrying the deleted work.
    ...
    def removeCredentialEscrows(self, said):
        """Remove retry-only state for one uncommitted credential SAID."""
        removed = []
        for name, escrow in (
        ...
        if self.saved.get(keys=(said,)) is None:
            self.creds.rem(keys=(said,))
            self.cancs.rem(keys=(said,))
